...
bool ScriptMethods::scrRestart(void)
{
	...
	frame[stack].index = 0;
        if(frame[stack].script->access == scrFUNCTION)
...
bool ScriptMethods::scrCounter(void)
{
	...
	while(NULL != (cp = getOption(NULL)))
	{
		...
		snprintf(sym->data, sym->size + 1, "%ld", ival);
		sym->type = symCOUNTER;
	}
	if(err)
		error(err);
