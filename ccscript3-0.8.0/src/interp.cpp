...
Script::Symbol *ScriptInterp::mapDirect(const char *id, unsigned short size)
{
	...
	{
		//cp = strchr(frame[stack].script->filename, '.');
		strcpy(cp,strchr(frame[stack].script->filename, '.'));
...
script:
	...
	if(pvt && scr->access == scrPRIVATE)
	{
		logmissing(label, "access", "script");
		error("script-private");
		return true;
	}
	if(!isfun && !fun && scr->access == scrFUNCTION && scr != frame[stack].script)
	{
		logmissing(label, "access", "script");
...
const char *ScriptInterp::getContent(const char *opt)
{
	...
	if(*opt == '#')
	{
		...
		case symCONST:
		case symNORMAL:
			snprintf(tmp, 11, "%d", strlen(sym->data));
			return tmp;
		case symINITIAL:
			return "0";
		case symPROPERTY:
