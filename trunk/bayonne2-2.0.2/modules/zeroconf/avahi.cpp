...
class Service : public BayonneService, public Keydata
{
private:
...
Service::Service() :
BayonneService(0, 0), 
Keydata("/bayonne/module/zeroconf")
{
	...
	if(Bayonne::getUserdata())
	        load("~bayonne/zeroconf");

	load(defkeys);
#ifdef	WIN32
	loadFile("C:\\Program Files\\GNU Telephony\\Bayonne Config\\zeroconf.ini", "tcpmon");
#endif
	name = avahi_strdup(getLast("name"));
	group = NULL;
	client = NULL;
}
