...
public:
	...
	/**
 	 * This is an internal call class.
	 */
	class __EXPORT Call : public Audio
	{
	private:
		volatile unsigned refcount;

	public:
		volatile time_t call_start, call_connect;
		char call_from[65], call_to[65], call_reason[16];
		char call_code[16], call_id[16];
		calltype_t call_type;
		const char *call_encoding;

		Call(BayonneSession *target, const char *from);
		void detach(const char *reason = NULL);
		void attach(void);
		void start(void);
	};
	...
	static BayonneSession *startDialing(const char *dial, 
		const char *name, const char *caller, const char *display, BayonneSession *parent = NULL);
 ...
 */
class __EXPORT BayonneConfig : public Keydata
{
private:
...
public:
	...
	virtual bool isReachable(const char *proxy);
         ...
         */
        virtual unsigned getRegistration(regauth_t *data, unsigned count);
};	
...
public:
	static void reportCall(Call *call);
...
private:
	...
	friend class __EXPORT Bayonne::Call;
...
//	Name *getScript(const char *scr);
	...
	/**
	 * Set call reference pointer.  If previous call is set, then
	 * it is detached.  Use NULL to clear current call if present.
	 */
	void setCall(Call *call, const char *reason = NULL);

	/**
	 * Make a new call, typically in a child process...
	 */
	void makeCall(const char *from);
...
public:
	 ...
	 */
	inline bool signalScript(signal_t signal)
		{return ScriptInterp::signal(signal);};
...
public:
	struct {
		...
		const char *agent_id;
		const char *protocol;
		const char *authorization;	// user:secret
		rpcauth_t authorized;	// authorized by web server...
	}	transport;
	...
	const char *getIndexed(unsigned short param, unsigned short offset);
