...
BayonneBinder("ivrscript1")
{
	...
	server->setValue("connect.conf", CONFIG_FILES "/connect.conf");
	server->setValue("feature.conf", CONFIG_FILES "/feature.conf");
};
...
#ifdef	WIN32
	prefix = "C:\\Program Files\\GNU Telephony\\Bayonne IVRScript1";
#else
	prefix = BAYONNE_FILES "/ivrscript1";
#endif
	if(prefix)
	{
		if(isDir(prefix))
