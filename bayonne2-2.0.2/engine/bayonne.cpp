...
Bayonne::Call::Call(BayonneSession *dest, const char *from)
{
	time_t buf;
	const char *dn = dest->getSymbol("session.dialed");
	if(!dn)
		dn = "unknown";

	strcpy(call_id, dest->var_sid);
	setString(call_from, sizeof(call_from), from);
	setString(call_to, sizeof(call_to), dn);
	call_connect = 0;
	call_encoding = dest->audioEncoding();
	refcount = 1;
	call_type = dest->type;
	call_start = time(&buf);
}

void Bayonne::Call::detach(const char *reason)
{
	if(reason && !reason[0])
		setString(call_reason, sizeof(call_reason), reason);

	--refcount;
	if(!refcount)
		BayonneBinder::reportCall(this);
}

void Bayonne::Call::attach(void)
{
	++refcount;
}

void Bayonne::Call::start(void)
{
	time_t buf;
	if(!call_connect)
		call_connect = time(&buf);
}
...
send:
	...
	if(trap_so4 != INVALID_SOCKET && trap_count4)
		for(idx = 0; idx < trap_count4; ++idx)
			sendto(trap_so4, (char*)(LPCTSTR)buf, len, 0, (struct sockaddr *)&trap_addr4[idx], alen);
...
BayonneSession *Bayonne::startDialing(const char *dial, const char *name, const char *caller, const char *display, BayonneSession *parent)
{
	ScriptImage *img;
