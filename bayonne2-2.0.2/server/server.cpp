...
static void loading(void)
{
	...
	keyurl.setEnv("proxy");
	keyurl.setEnv("local");
	keyurl.setEnv("userid");
	keyurl.setEnv("secret");
	keyurl.setEnv("prefix");

	keysmtp.setEnv("sendmail");
	keysmtp.setEnv("errors");
	keysmtp.setEnv("sender");
	keysmtp.setEnv("userid");
	keysmtp.setEnv("secret");
	...
	if(!driver)
	{
		slog.critical("no driver loaded; enable in server.conf...");
		stop(-1);
	}
